...
fun main() {
    val rounds =  getResourceAsLines("/02 - RockPaperScissors.txt")
    val result1 = rounds.map { Round.fromStringToSimpleRound(it) }.map { it.calculateRound() }.sum()
    val result2 = rounds.map { Round.fromStringToSecondRound(it) }.map { it.calculateRound() }.sum()
